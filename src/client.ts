...
// import { buildZkJudgeContract } from './generated/zk_judge';
// import { MidnightProvider } from '@midnight-ntwrk/midnight-js';

export interface UserPrivateData {
    attr1: number; // e.g., Credit Score
    attr2: number; // e.g., Monthly Income (scaled)
    attr3: number; // e.g., On-time payment history (scaled)
}
 ...
 */
export async function evaluateAndSubmitProof(userData: UserPrivateData, targetThreshold: number) {
...
if (require.main === module) {
    ...
    const validData: UserPrivateData = { attr1: 50, attr2: 100, attr3: 10 }; 
    ...
    evaluateAndSubmitProof(validData, threshold).then(() => {
        ...
        const invalidData: UserPrivateData = { attr1: 30, attr2: 50, attr3: 5 };
